...
export default function Courses(props) {
  const images = importAll(
    require.context("../images/course", false, /\.(png|jpe?g|svg)$/)
  );
  ...
  for (let course in CourseData) {
    CoursesElements.push(
      <div className="CourseContainer" key={CourseData[course].FullName}>
        <img
          src={images[CourseData[course].Image]}
          alt="mca class"
        ...
        <div className="CourseCaption">
          ...
          <div className="CaptionItem">
            <img
              src={specializationIco}
              alt="duration"
              className="CaptionItemIcon"
            />
    ...
    if (CourseData[course].Syllabus && CourseData[course].Syllabus.length > 0) {
      let syllabusElement = [];
      ...
      for (let syllabus of CourseData[course]["Syllabus"]) {
        ...
        for (let subject of syllabus.Structure) {
          let subjectNames = subject.Courses.map((courseName) => (
            <h3 key={courseName}> - {courseName}</h3>
          ));
      ...
      CurriculumElements.push(
        <div key={course}>
          ...
          <div className="CurriculumContainer">
            <p>Syllabus information coming soon.</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div>
      <div id="CoursePage" className="CoursePageSection">
        ...
        <div className="CoursesList">{CoursesElements}</div>
      </div>
      <div className="CoursePageSection">
        ...
        <div className="CurriculumBody">{CurriculumElements}</div>
      </div>
    </div>
  );
}
