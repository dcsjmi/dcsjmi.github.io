...
export default function GlobalHeader(props) {
  ...
  useEffect(() => {}, [props.shouldCollapse]);
  ...
  for (let pageLink in PageRoutes) {
    PageRouteButtons.push(
      <Link
        to={PageRoutes[pageLink]}
        key={pageLink}
        className="NavLink"
        onClick={ToogleNav}
        aria-selected={PageRoutes[pageLink].match(activeTab) ? true : false}
      >
        {pageLink}
      </Link>
    );
  ...
  return (
    <div className="Header">
      <div
        className={
          props.shouldCollapse ? "HeaderBody CollapseHeader" : "HeaderBody"
        }
      >
        <div className="DrawerLogoContainer" onClick={ToogleNav}>
          <img src={navOpenIco} alt="Open Drawer" ref={DrawerRef} />
        </div>
